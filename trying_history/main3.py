...
def multi_scale_edge_template_matching(screenshot, template, scale_ranges, step=0.1, threshold=0.8):
    ...
    for scale in np.arange(scale_ranges[0], scale_ranges[1], step):
        resized_template = cv2.resize(edge_template, (0, 0), fx=scale, fy=scale)  # Resizing template
